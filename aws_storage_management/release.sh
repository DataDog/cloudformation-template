...
response=$(aws s3api head-object \
    --bucket "${BUCKET}" \
    --key "aws_storage_management/${VERSION}/main.yaml" > /dev/null 2>&1)

if [[ ${?} -eq 0 ]]; then
    echo "S3 bucket path ${BUCKET}/aws_storage_management/${VERSION} already exists. Please up the version."
    exit 1
fi
set -e
...
for i in *.yaml; do
    [ -f "$i" ] || break
    echo "About to upload $i to s3://${BUCKET}/aws_storage_management/${VERSION}/$i"
done
...
if [ "$PRIVATE_TEMPLATE" = true ] ; then
    ...
    aws s3 cp . s3://${BUCKET}/aws_storage_management/${VERSION} --recursive --exclude "*" --include "*.yaml" --acl bucket-owner-full-control
fi
