...
response=$(aws s3api head-object \
    --bucket "${BUCKET}" \
    --key "aws_cloud_cost_cur2/${VERSION}/main.yaml" > /dev/null 2>&1)

if [[ ${?} -eq 0 ]]; then
    echo "S3 bucket path ${BUCKET}/aws_cloud_cost_cur2/${VERSION} already exists. Please up the version."
    exit 1
fi
set -e
...
set +e
response=$(grep -q "aws_cloud_cost_cur2/${VERSION}/main.yaml" README.md)
if [[ ${?} -ne 0 ]]; then
...
for i in *.yaml; do
    [ -f "$i" ] || break
    echo "About to upload $i to s3://${BUCKET}/aws_cloud_cost_cur2/${VERSION}/$i"
done
...
if [ "$PRIVATE_TEMPLATE" = true ] ; then
    aws s3 cp . s3://${BUCKET}/aws_cloud_cost_cur2/${VERSION} --recursive --exclude "*" --include "*.yaml"
else
    aws s3 cp . s3://${BUCKET}/aws_cloud_cost_cur2/${VERSION} --recursive --exclude "*" --include "*.yaml" --acl bucket-owner-full-control
fi
echo "Done uploading the template, and here is the CloudFormation quick launch URL"
echo "https://console.aws.amazon.com/cloudformation/home#/stacks/create/review?stackName=datadog-aws-cloud-cost&templateURL=https://${BUCKET}.s3.amazonaws.com/aws_cloud_cost_cur2/${VERSION}/main.yaml"
