...
# The <= v4.13 inline trigger deletes un-suffixed policies when removed during an upgrade.
# Keep replacement policies on v2 names so that teardown cannot delete newly attached policies.
...
# Remove legacy base policies before attaching v2 policies to avoid exceeding the role quota.
# Instrumentation was not released under legacy names, so it needs no equivalent cleanup list.
...
def _broad_policy_document(actions):
    return {
...
def fetch_instrumentation_policy_documents(api_url):
    ...
    return [_broad_policy_document(chunk) for chunk in permission_chunks]
...
def attach_standard_permissions(iam_client, role_name):
    ...
    iam_client.put_role_policy(
        ...
        PolicyDocument=json.dumps(_broad_policy_document(permissions), separators=(',', ':')),
    )
...
def _serialize_policy_document(policy_document):
    ...
    if policy_document.get("Version") != POLICY_DOCUMENT_VERSION:
...
def _create_and_attach_policy(iam_client, role_name, policy_name, actions):
    policy_arn = _create_policy(iam_client, policy_name, _broad_policy_document(actions))
...
def attach_instrumentation_permissions(iam_client, role_name, account_id, partition, datadog_site, resource_types, previous_resource_types, fail_on_error=False):
    # Role creation treats instrumentation as best-effort; the add-on fails when its only task
    # cannot complete. Replacement stages policy versions before activation so it can roll back.
...
def handle_delete(event, context):
    ...
    manage_base_permissions = _is_true(props.get('ManageBasePermissions', 'true'))
...
def handle_create_update(event, context):
    ...
    partition = props.get('Partition', 'aws')
    manage_base_permissions = _is_true(props.get('ManageBasePermissions', 'true'))
    fail_on_instrumentation_error = _is_true(props.get('FailOnInstrumentationError', 'false'))
    should_install_security_audit_policy = _is_true(props['ResourceCollectionPermissions'])
