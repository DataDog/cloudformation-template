...
def attach_instrumentation_permissions(iam_client, role_name, account_id, partition, datadog_site, resource_types, previous_resource_types, fail_on_error=False):
    ...
    # Only now that the fetch has succeeded do we remove the existing instrumentation policies —
    # both this template's v2 names and any legacy un-suffixed ones from a pre-extraction upgrade.
    # Deferring until here is what keeps a transient fetch failure above from leaving the role
    # with no instrumentation permissions.
    cleanup_instrumentation_policies(iam_client, role_name, account_id, partition)
    _cleanup_chunked_policies(iam_client, role_name, account_id, partition, LEGACY_PREFIX_INSTRUMENTATION)
    for i, chunk in enumerate(permission_chunks):
