...
def _action_covered_by_standard(action, resource, condition, standard_index):
    for action_set, standard_resource, standard_condition in standard_index:
        ...
        if not _resource_covers(standard_resource, resource):
            continue
        if not _condition_covers(standard_condition, condition):
            continue
        return True
    return False
...
def chunk_statements(statements, max_size=MAX_MANAGED_POLICY_SIZE):
    ...
    for statement in statements:
        statement_size = len(json.dumps(statement, separators=(',', ':')))
        candidate_size = current_size + statement_size + (1 if current else 0)
        if current and candidate_size > max_size:
            chunks.append(current)
            current = [statement]
            current_size = base_size + statement_size
        else:
...
def attach_resource_collection_permissions(iam_client, role_name):
    ...
    for i, chunk in enumerate(permission_chunks):
        _create_and_attach_statement_policy(
            ...
            [{"Effect": "Allow", "Action": chunk, "Resource": "*"}],
        )
