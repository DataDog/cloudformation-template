...
# The "-v2" suffix on the standard and resource-collection names is load-bearing, not cosmetic.
# Extracting this custom resource out of datadog_integration_role.yaml means an in-place upgrade
# from a release that still had the inline trigger (<= v4.13) removes that old trigger, whose
# Delete handler deletes the standard/resource-collection policies by their un-suffixed names.
# CloudFormation can run that delete after this nested stack has re-attached them, which would
# leave the role without those permissions. The v2 names dodge that: the old handler only ever
# touches the old-named policies. Instrumentation needs no such suffix — that feature is
# unreleased, so no field stack carries instrumentation policies for the old handler to collide
# with, and the inline trigger is gone for good from v4.14 onward.
...
# (<= v4.13). We clean these up ourselves before attaching the v2 policies so the two generations
# never sit attached at once (IAM caps managed policies per role, default 10); the old trigger's
# own Delete handler then no-ops against names that are already gone.
...
def cleanup_legacy_base_policies(iam_client, role_name, account_id, partition, max_policies=10):
    # Remove the un-suffixed standard + resource-collection policies left by the inline trigger of
    # releases that predate this nested template, so they are gone before the v2 standard/RC
    # policies are attached (keeps both generations from piling up against the IAM managed-policy
    # limit during an in-place upgrade). Only the role-creation path calls this; the add-on must
    # not touch the standard/RC policies the role stack owns. The legacy *instrumentation* policies
    # are handled in attach_instrumentation_permissions instead, where cleanup is deferred until a
    # successful fetch so a transient API failure can't leave the role with no instrumentation.
...
def attach_instrumentation_permissions(iam_client, role_name, account_id, partition, datadog_site, resource_types, previous_resource_types, fail_on_error=False):
    ...
    # Only now that the fetch has succeeded do we remove the existing instrumentation policies, so
    # a transient fetch failure above leaves the previously-attached policies in place.
    cleanup_instrumentation_policies(iam_client, role_name, account_id, partition)
    for i, chunk in enumerate(permission_chunks):
