...
embed_python_source() {
    local template="$1"
    local source="$2"
    local placeholder="${3:-ZIPFILE_PLACEHOLDER}"

    DD_QUICKSTART_SOURCE_PLACEHOLDER="<${placeholder}>" perl -i -pe '
        BEGIN {
            $p = do { local $/; <STDIN> };
            $placeholder = $ENV{"DD_QUICKSTART_SOURCE_PLACEHOLDER"};
        }
        /^(\s+)\Q$placeholder\E\s*$/ && (
            $_ = join("\n", map { $1 . $_ } split(/\n/, $p)) . "\n"
        )
...
embed_python_source datadog_integration_permissions.yaml accept_operator_subscription.py ACCEPT_OPERATOR_SUBSCRIPTION_SOURCE
