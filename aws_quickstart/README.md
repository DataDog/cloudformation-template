...
- An EventBridge pipeline (connection, API destination, invocation role, and per-service CloudTrail rules) that forwards resource lifecycle events to Datadog so the Agent install feature can react in real time.
  - Deployed only when `InstrumentationResourceTypes` is set. A rule is created for each listed resource type — listing `aws:ec2:instance` deploys the EC2 rule, listing `aws:eks:cluster` deploys the EKS rule.
